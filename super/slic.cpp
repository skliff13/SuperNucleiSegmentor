...
/*
 * Give the pixels of each cluster the gray level corresponding to cluster index. The specified colour
 * is the mean RGB colour per cluster.
 *
 * Input : The target CV_32S (int) image (IplImage*).
 * Output: -
 */
void Slic::colour_with_cluster_indices(IplImage *image) {
    /* Fill in. */
    for (int i = 0; i < image->width; i++) {
        for (int j = 0; j < image->height; j++) {
            int index = clusters[i][j];
